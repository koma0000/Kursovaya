...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Kursovaya Khairzamanov", nullptr, nullptr);
    if (window == nullptr)
    {
    ...
    Shader shaderGBuffer("g_buffer.vs", "g_buffer.fs");
    Shader shaderLighting("defShad.vs", "defShad.fs");
    Shader shaderLightBox("lightBox.vs", "lightBox.fs");

    Model testObj("Content\\obj\\Table\\Wood_Table.obj", vec3(0, 0, 0), shaderGBuffer);
    Model backpack("Content\\obj\\backpack\\backpack.obj", vec3(0, 0, 0), shaderGBuffer);

    Model dog;
    dog = Model("Content\\obj\\Dog\\dog.obj", vec3(5.5, 0, 7), shaderGBuffer);
    dog.Scale(vec3(0.03f, 0.03f, 0.03f));
    ...
    std::vector<glm::vec3> bacbackpacks;
    for (int i = 0; i < 3; i++) {
        bacbackpacks.push_back(glm::vec3(5* i, 0.5, 2.0));
    }
    ...
    for (int i = 0; i < 10; i++) {
        objectPositions.push_back(glm::vec3(1.5 *i , 0.5, 10.0));
    }
    ...
    for (unsigned int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        float xPos = ((rand() % 100) / 100.0) * 18.0 - 1.0;
        float yPos;
        if (i % 2 == 0) {
           yPos = 3;
        }
        ...
        float zPos = ((rand() % 100) / 100.0) * 18.0 - 3.0;
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      dog.Draw();
        ...
        for (unsigned int i = 0; i < bacbackpacks.size(); i++)
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, bacbackpacks[i]);
            model = glm::scale(model, vec3(1));
            backpack.setModel(model);
            backpack.Draw();
        } 
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        for (unsigned int i = 0; i < lightPositions.size(); i++)
        {
            ...
            shaderLighting.setFloat("lights[" + std::to_string(i) + "].Radius", radius);
        }
